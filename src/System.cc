...
namespace ORB_SLAM2 {
    ...
    System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor, const bool bUseViewer)
        ...
        /// 下面是对关键变量的初始化定义
        ///        指针成员变量与对象成员变量辨析
        ///        ①指针成员变量指向另一个位置的对象，一般是与当前class平级的一个class，当前对象消失指向的对象不一定消失
        ///        ②对象成员变量在当前对象中，一般属于当前对象，当前对象消失对象成员变量也消失
...
/*        if (!bVocLoad) {
        ...
        // 主线程跟踪线程  Initialize the Tracking thread (it will live in the main thread of execution, the one that called this constructor)
