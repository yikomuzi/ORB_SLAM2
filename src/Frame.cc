...
namespace ORB_SLAM2 {
    ...
    // 用于双目模式,在构造函数里就自动进行对图片帧的处理，提取特征点
...
//            cout << mvKeys[i].pt << endl;
//        }
//        cout << "------------------------------------------------" << endl;
        cout << "mvKeysRight.size(): " << mvKeysRight.size() << endl;
//        for (int i = 0; i < mvKeysRight.size(); i++) {
//            cout << mvKeysRight[i].pt << endl;
//        }
        ...
        /// 2. 特征点去畸变（双目无需）
        UndistortKeyPoints();  // 由于是双目模式（畸变已预先校正），直接执行mvKeysUn = mvKeys;

        /// 3. 计算mvuRight（左目特征点在右目中匹配特征点的横坐标，左右目匹配特征点的纵坐标相同)，mvDepth（特征点深度）
        ComputeStereoMatches();
        cout << "mvuRight.size(): " << mvuRight.size() << endl;
        cout << "mvDepth.size(): " << mvDepth.size() << endl;
//        for (int i = 0; i < mvuRight.size(); i++) {
//            cout << mvuRight[i] << endl;
//        }
    ...
    void Frame::ExtractORB(int flag, const cv::Mat &im) {
        // 特征点被存进std::vector<cv::KeyPoint> mvKeys中，描述子被存入cv::Mat mDescriptors中
        if (flag == 0)
            (*mpORBextractorLeft)(im, cv::Mat(), mvKeys, mDescriptors);  // 【关键】重载括号运算符实现特征点提取
        else
