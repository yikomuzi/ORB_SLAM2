...
//        unique_lock<mutex> lock(mpMap->mMutexMapUpdate);
        ...
        if (mState == NOT_INITIALIZED) {
            ...
            if (mState != OK)
                return;
        } else {                                  // 从第二帧开始从这里执行. 若系统已初始化,就进行跟踪(或重定位)
            ...
            if (mVelocity.empty() || mCurrentFrame.mnId < mnLastRelocFrameId + 2) {
                ...
                bOK = TrackWithMotionModel();                   // 根据运动模型跟踪
                if (!bOK)
                    bOK = TrackReferenceKeyFrame();             // 运动模型跟踪失败,根据参考帧进行跟踪
            }
...
//            mCurrentFrame.mpReferenceKF->ComputeBoW();  // 彻底去耦合局部建图
    ...
    bool Tracking::TrackWithMotionModel() {
        cout << "[Tracking] Tracking::TrackWithMotionModel()--------------" << endl;
    ...
    bool Tracking::TrackLocalMap() {
        ...
        cout << "[Tracking] Tracking::TrackLocalMap()-------------" << endl;
    ...
    void Tracking::CreateNewKeyFrame() {
        ...
        mpLocalMapper->InsertKeyFrame(pKF);  // 局部建图线程启动
